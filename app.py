...
# Reads securely from Streamlit Cloud Secrets in production or local environment
...
with st.sidebar:
    ...
    if not openai.api_key:
        user_key = st.text_input("Enter OpenAI API Key manually to override:", type="password")
        if user_key:
            openai.api_key = user_key
            st.success("API Credentials bound successfully.")
        else:
            st.warning("Provide a key above or add it to secrets to execute generation.")
            
    st.markdown("---")
    st.caption("Powered by Open Source Python & OpenAI Streamlit Pipelines.")
...
with col_main:
    ...
    # Core AI Logic Generator Function
    if st.button(f"Generate Content for {selected_chapter}"):
        if not openai.api_key:
            st.error("Error: Missing OpenAI API access token credentials.")
        else:
            with st.spinner(f"Computing deep data analysis matrix for {selected_chapter}..."):
                try:
                    ...
                    # OpenAI API call matching the standard v1.0.0+ implementation
                    response = openai.chat.completions.create(
                        model="gpt-4o",  # High-fidelity synthesis model
                        messages=[
